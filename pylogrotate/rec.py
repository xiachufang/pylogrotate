...
class Reader(object):
    ...
    def __init__(self, log_home, log_tag, split_datetime, subdir_name):
        ...
        self.iter = self.load_need_to_split_file(log_tag, split_datetime, dirpath)

    @classmethod
    def load_need_to_split_file(cls, log_tag, split_datetime, dirpath):
        ...
        split_file_path = sorted(datetime_distance_dict.items(), key=itemgetter(1))[0][0]
        if not os.path.exists(split_file_path):
            raise OSError("HAVE NOT THIS FILE")
        for line in cls.read_gz_file(split_file_path):
            yield line
    ...
    for subdir_name in os.listdir(log_home):
        ...
        for dt, log_line in reader:
            ...
            if dt - start_time > datetime.timedelta(minutes=5):
                with Writer(log_home=log_home, log_tag=log_tag, rotated_time=dt, subdir_name=subdir_name) as f:
